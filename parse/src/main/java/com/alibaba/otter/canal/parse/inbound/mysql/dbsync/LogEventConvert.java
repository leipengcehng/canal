    ...
    private boolean parseOneRow(RowData.Builder rowDataBuilder, RowsLogEvent event, RowsLogBuffer buffer, BitSet cols,
        ...
        for (int i = 0; i < columnCnt; i++) {
            ...
            if (existRDSNoPrimaryKey && i == columnCnt - 1 && info.type == LogEvent.MYSQL_TYPE_LONGLONG) {
                // 不解析最后一列
                buffer.nextValue(info.type, info.meta, false, null);
                continue;
            }
            ...
            if (tableMeta != null && !tableError) {
                ...
                if (fieldMeta.getColumnCharset() != null) {
                    columnBuilder.setCharSetName(fieldMeta.getColumnCharset());
                }
            ...
            buffer.nextValue(info.type, info.meta, isBinary, columnBuilder.getCharSetName());
            if (existRDSNoPrimaryKey && i == columnCnt - 1 && info.type == LogEvent.MYSQL_TYPE_LONGLONG) {
                // 不解析最后一列
                continue;
            }
